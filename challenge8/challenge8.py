...
class Challenge8(unittest.TestCase):
    ...
    def test_challenge8(self):
        ...
        for query in range(len(queries)):
            ...
            print("OLD PAYLOAD: " + totalElements + " results for '" + queries[query] + "'")

            newPayload = self.getNewPayload(queries[query])
            response = requests.post(url, data=json.dumps(newPayload), headers=headers)
            totalElements = str(response.json()['data']['results']['totalElements'])
            print("NEW PAYLOAD: " + totalElements + " results for '" + queries[query] + "'")
            print()
    ...
    def getNewPayload(self, searchQuery):
        newPayload = {
            "draw": "1",
            "columns[0][data]": "0",
            "columns[0][name]": "",
